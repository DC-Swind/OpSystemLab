...
extern volatile int timers;  
extern int last_key;
extern bool status_up;
extern bool status_down;
extern bool status_left;
extern bool status_right;
extern bool status_j;
extern bool status_k;
extern bool status_i;
extern bool status_u;

static uint8_t temp_draw_buffer_r[256*240];
static uint8_t temp_draw_buffer_g[256*240];
static uint8_t temp_draw_buffer_b[256*240];
...
void wait_for_frame()
{
    ...
    */
    prepare_buffer();
    memset(temp_draw_buffer_r, 0 ,256*240);
    memset(temp_draw_buffer_g, 0 ,256*240);
    memset(temp_draw_buffer_b, 0 ,256*240);
...
void nes_set_bg_color(int r, int g, int b, int c)
{
 ...
 int cc = ((r>>5)<<5) + ((g>>5)<<2) + (b>>6);
    int i,j;
    for (i = 0; i<320; i++)
        for (j = 0; j<200; j++) draw_pixel(i,j,cc);
}
...
void nes_draw_pixel(Pixel *p)
{
    ...
    */
    temp_draw_buffer_r[(p->y<<8)+p->x] = p->r;
    temp_draw_buffer_g[(p->y<<8)+p->x] = p->g;
    temp_draw_buffer_b[(p->y<<8)+p->x] = p->b;

   // int c = ((p->r>>5)<<5) + ((p->g>>5)<<2) + (p->b>>6);
    //draw_pixel(p->x, p->y, c);
}
...
void nes_flip_display()
{
    ...
    // al_flip_display();
    //vtx_sz = 0;
    //draw_string(itoa(last_key), 0, SCR_HEIGHT - 8, 10);
    int i,j,x,y;
    double tmpx,tmpy,u,v,r,g,b;
    for (i = 0; i < SCR_WIDTH; i++)
        for (j = 0; j<SCR_HEIGHT; j++){
            tmpx = (double)(i+1) * 0.8 -1;
            tmpy = (double)(j+1) * 1.2 -1;
            x = (int)tmpx;
            y = (int)tmpy;
            u = tmpx - x;
            v = tmpy - y;
            r = (1-u)*(1-v)*temp_draw_buffer_r[(y<<8)+x] + (1-u)*v*temp_draw_buffer_r[((y+1)<<8)+x] + (1-v)*u*temp_draw_buffer_r[(y<<8)+x+1] + u*v*temp_draw_buffer_r[((y+1)<<8)+x+1]; 
            g = (1-u)*(1-v)*temp_draw_buffer_g[(y<<8)+x] + (1-u)*v*temp_draw_buffer_g[((y+1)<<8)+x] + (1-v)*u*temp_draw_buffer_g[(y<<8)+x+1] + u*v*temp_draw_buffer_g[((y+1)<<8)+x+1]; 
            b = (1-u)*(1-v)*temp_draw_buffer_b[(y<<8)+x] + (1-u)*v*temp_draw_buffer_b[((y+1)<<8)+x] + (1-v)*u*temp_draw_buffer_b[(y<<8)+x+1] + u*v*temp_draw_buffer_b[((y+1)<<8)+x+1]; 
            int c = (((int)r>>5)<<5) + (((int)g>>5)<<2) + ((int)b>>6);
    ...
    */
    ...
    switch (b)
	{
        case 0: // On / Off
            return 1;
        case 1: // A
            if (status_k) return 1; else return 0;
        case 2: // B
            if (status_j) return 1; else return 0;
        case 3: // SELECT
            if (status_u) return 1; else return 0;
        case 4: // START
            if (status_i) return 1; else return 0;
        case 5: // UP
            if (status_up)return 1; else return 0;
        case 6: // DOWN
            if (status_down) return 1; else return 0;
        case 7: // LEFT
            if (status_left) return 1; else return 0;
        case 8: // RIGHT
            if (status_right) return 1; else return 0;
        default:
            return 1;
    }
