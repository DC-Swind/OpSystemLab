...
cook(Console *c) {
	...
	do {
		...
		if (c->r == c->f) printf("cooked buffer full");
	} while (*p ++ != 0);
	c->lbuf[c->i = 0] = 0;
	cr(c); lf(c);
	if (c->rtop != 0) {
...
consl_accept(Console *c, char ch) {
	...
	for (i = cc + 1; i > 0; i --) {
		...
		if (c->i + i >= LBUF_SZ) printf("line buffer full");
	...
	if (c->i >= LBUF_SZ) printf("line buffer full");
	c->lbuf[c->i ++] = ch;
	consl_sync(c);
}
